...
/*
    ...
    NOTE:
    - to do template <typename ANY_TYPE> RotaryEncoderAdvanced(ANY_TYPE varieble) 
*/
/**************************************************************************/
RotaryEncoderAdvanced::RotaryEncoderAdvanced(uint8_t encoderA, uint8_t encoderB, uint8_t encoderButton, float stepsPerClick, float minValue, float maxValue) : RotaryEncoder(encoderA, encoderB, encoderButton)
{
  _stepsPerClick = stepsPerClick;
  _minValue      = minValue;
  _maxValue      = maxValue;
}
...
*/
/**************************************************************************/
float RotaryEncoderAdvanced::getValue()
{
  float value = (float)_counter * _stepsPerClick;

  if (value > _maxValue)      //make shure that "value" never gets higher "maxValue"
  {
    value = _maxValue;
    setValue(value);
  }
  else if (value < _minValue) //make shure that "value" never gets lower  "minValue"
  {
    value = _minValue;
    setValue(value);
  }
...
*/
/**************************************************************************/
void RotaryEncoderAdvanced::setValue(float value)
{
